...
{
	// variable is atomic therefore does not require a mutex
    return m_connected;
}
...
{	
    ...
    m_cond.wait(locker, [&]() { 
		Log::Debug(DEBUG_THREAD_BLOCKING, "Inside send() condtion"); 
		// wake condition 
		return (((m_available >= len) && m_connected) || m_runCommand); 
    });
    Log::Debug(DEBUG_THREAD_BLOCKING, "Passed send() condtion (runCmd = %d)", m_runCommand); 
    ...
    Log::Debug(DEBUG_SERIAL, "Serial: Sending = %s\t(%u/%u)", gcode.c_str(), m_available, MAX_GRBL_BUFFER);
...
{	
   {    // lock the mutex
		...
		Log::Debug(DEBUG_CHAR_COUNTING, "Remaining buffer: %d/%d", m_available, MAX_GRBL_BUFFER);
   }
