...
Point3D::Point3D(Constraints* parent, Group group, double x, double y, double z) 
{
    paramX = parent->MakeParam(group, x);  // x
    paramY = parent->MakeParam(group, y);  // y
    parameterZ = parent->MakeParam(group, z);  // z
    
    entity = parent->MakeEntity([&](Slvs_hEntity id) {
        return Slvs_MakePoint3d(id, group, paramX, paramY, parameterZ);
    });
}

Normal::Normal(Constraints* parent, Group group, double ux, double uy, double uz, double vx, double vy, double vz) 
{
    double qw, qx, qy, qz;
...
Workplane::Workplane(Constraints* parent, Group group, Point3D& origin, Normal& normal)
{
    entity = parent->MakeEntity([&](Slvs_hEntity id) {
...
Axis::Axis(Constraints* parent, Group group, double x, double y, double z) 
    :  origin(parent, group, x, y, z), normal(parent, group), plane(parent, group, origin, normal)
{}

Point2D::Point2D(Constraints* parent, Group group, Axis& axis, double x, double y) 
{
    paramX = parent->MakeParam(group, x);  // x
    paramY = parent->MakeParam(group, y);  // y
...
Distance::Distance(Constraints* parent, Group group, Axis& axis, double distance) 
{
...
Line::Line(Constraints* parent, Group group, Axis& axis, double x0, double y0, double x1, double y1)
...
Arc::Arc(Constraints* parent, Group group, Axis& axis, double xC, double yC, double x0, double y0, double x1, double y1)
...
Circle::Circle(Constraints* parent, Group group, Axis& axis, double xC, double yC, double r)
