...
void GRBLSystem::decodeCoords(const std::string& msg) 
{
    ...
    if(!param.compare("G54")) 
        coords.m_vals.workCoords[0] = StringToVec3(num);
    else if(!param.compare("G55")) 
        coords.m_vals.workCoords[1] = StringToVec3(num);
    else if(!param.compare("G56")) 
        coords.m_vals.workCoords[2] = StringToVec3(num);
    else if(!param.compare("G57")) 
        coords.m_vals.workCoords[3] = StringToVec3(num);
    else if(!param.compare("G58")) 
        coords.m_vals.workCoords[4] = StringToVec3(num);
    else if(!param.compare("G59")) 
        coords.m_vals.workCoords[5] = StringToVec3(num);
    // [G28:1.000,2.000,0.000]  / [G30:4.000,6.000,0.000]
    else if(!param.compare("G28")) 
        coords.m_vals.homeCoords[0] = StringToVec3(num);
    else if(!param.compare("G30")) 
        coords.m_vals.homeCoords[1] = StringToVec3(num);
    // [G92:0.000,0.000,0.000]
    else if(!param.compare("G92")) 
        coords.m_vals.offsetCoords = StringToVec3(num);
    // [TLO:0.000]    
    else if(!param.compare("TLO")) 
        coords.m_vals.toolLengthOffset = stof(num);
    // [PRB:0.000,0.000,0.000:0]
    else if(!param.compare("PRB")) {
        coords.m_vals.probeOffset = StringToVec3(msg.substr(5, msg.length()-8));
...
void GRBLSystem::decodeStatus(const std::string& msg) 
{
    ...
    for (int i = segs.size()-1; i >= 0; i--) 
    {
        ...
        // WPos = MPos - WCO
            ...
            status.m_vals.MPos = StringToVec3(segs[i].substr(5));
            status.m_vals.WPos = status.m_vals.MPos - status.m_vals.WCO;
        }
        else if(segs[i].substr(0, 4) == "WPos") {
            status.m_vals.WPos = StringToVec3(segs[i].substr(5));
        ...
        // the current work coordinate system, G92 offsets, and G43.1 tool length offset
            ...
            status.m_vals.WCO = StringToVec3(segs[i].substr(4));
        }
        ...
        {
            Vec3 ov = StringToVec3(segs[i].substr(3));
...
void GRBL::sendJog(const Vec3& p, int feedrate) {
...
int GRBL::getLastItem(GCodeItem& item) {
    return gcList.getLastItem(item);
}
// returns items in the gcode list for viewing
const GCodeItem& GRBL::getGCodeItem(uint index) {
    return gcList.getItem(index);
}
    // returns items in the gcode list for viewing
uint GRBL::getGCodeListSize() {
...
int GRBL::processResponse(const std::string& msg)
{
    ...
    {  // retrieve error code
        ...
        if(getErrMsg(errCode, &errName, &errDesc)) 
            ...
            Log::Info("[Response] Error %d: %s (%s)", errCode, errName.c_str(), errDesc.c_str());
        return errCode;
    };
    ...
    {    // retrieve alarm code
        ...
        if(getAlarmMsg(alarmCode, &alarmName, &alarmDesc))
            ...
            Log::Info("[Response]ALARM %d: %s (%s)", alarmCode, alarmName.c_str(), alarmDesc.c_str());
    };
